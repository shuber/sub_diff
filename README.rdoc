...
But it also allows you to check if the replacement actually changed anything.

  replaced.changed? #=> true

For a closer look at the changes, you can enumerate thru the returned replacement object, yielding a <tt>Diff</tt> object for each iteration. Each <tt>Diff</tt> object also behaves like a <tt>String</tt> but includes a few additional methods.
